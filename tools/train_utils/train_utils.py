...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    # AMP dtype: BF16 if requested and supported, else FP16. GradScaler is only meaningful for FP16.
    amp_dtype_cfg = str(optim_cfg.get('AMP_DTYPE', 'bf16')).lower()
    use_bf16 = (amp_dtype_cfg == 'bf16') and torch.cuda.is_available() and torch.cuda.is_bf16_supported()
    amp_dtype = torch.bfloat16 if use_bf16 else torch.float16
    scaler = torch.amp.GradScaler('cuda', enabled=use_amp and not use_bf16, init_scale=optim_cfg.get('LOSS_SCALE_FP16', 2.0**16))
    ...
    #  -1 keeps the backbone frozen for the whole run; 
    #  default 0 trains end-to-end from the start.
    mm = model.module if hasattr(model, 'module') else model
    if hasattr(mm, 'set_freeze_epoch'):
        unfreeze_epoch = optim_cfg.get('UNFREEZE_BACKBONE_EPOCH', 0)
        frozen = mm.set_freeze_epoch(cur_epoch, unfreeze_epoch)
        if logger is not None and rank == 0:
            logger.info(f'Epoch {cur_epoch}: backbone {"FROZEN" if frozen else "UNFROZEN"} (unfreeze_epoch={unfreeze_epoch})')
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        # Keep the final loss in FP32 for numerically stable backward
        loss = loss.float()
        ...
        if rank == 0:
            ...
            if use_logger_to_record:
                if accumulated_iter % logger_iter_interval == 0 or cur_it == start_it or cur_it + 1 == total_it_each_epoch:
                    ...
                    if 'rl/reward_mean' in tb_dict:
                        log_text += f"  Reward: {tb_dict['rl/reward_mean']:.3f}"
            ...
            if time_past_this_epoch // ckpt_save_time_interval >= ckpt_save_cnt:
                ckpt_name = ckpt_save_dir / f'latest_model_ep{cur_epoch}_iter{accumulated_iter}'
                save_checkpoint(
