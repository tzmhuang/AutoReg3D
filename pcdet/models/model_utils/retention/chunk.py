...
def chunk_retention_fwd_kernel_o(
    ...
    d_s = tl.where(m_s, tl.math.exp2((o_i[:, None] - o_i[None, :]) * b_b), tl.math.exp2((o_i[None, :] - o_i[:, None]) * b_b))
...
def chunk_retention_bwd_kernel_dqkv(
    ...
    d_s = tl.where(m_s, tl.math.exp2((o_i[:, None] - o_i[None, :]) * b_b), tl.math.exp2((o_i[None, :] - o_i[:, None]) * b_b)) * scale
