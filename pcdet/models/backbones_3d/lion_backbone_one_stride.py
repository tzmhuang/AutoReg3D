...
# The 5 LION linear-operator blocks (Mamba / RWKV / RetNet / xLSTM / TTT) are
# imported lazily in `get_linear_operator` below so that importing this module
# (and therefore `pcdet.models`) does not require all of their CUDA / Triton
# dependencies — mamba_ssm + causal_conv1d for Mamba, pcdet.ops.wkv6 for RWKV,
# the Triton retention kernels for RetNet, etc. Only the operator selected by
# `BACKBONE_3D.OPERATOR.NAME` is actually imported.
...
class PatchMerging3D(nn.Module):
    ...
    def forward(self, x, coords_shift=1, diffusion_scale=4):
        ...
        x = self.sub_conv(x)
        # _dtype = x.features.dtype
        # with torch.autocast('cuda', enabled=False):
        #     x = x.replace_feature(x.features.to(torch.float32))
        #     x = self.sub_conv(x)
        #     # cast back
        #     x = x.replace_feature(x.features.to(_dtype))
...
def get_linear_operator(name):
    """Return the LION linear-operator block class for `name`, importing only
    the one actually requested. This keeps mamba_ssm / wkv6_cuda /
    causal_conv1d / retention Triton kernels / einops / timm from being
    required at module load — they are pulled in lazily and only for the
    operator the config selects."""
    if name == 'Mamba':
    ...
    if name == 'TTT':
        ...
        return TTTBlock
    raise ValueError(
        f"Unknown LION operator '{name}'. "
        f"Expected one of: Mamba, RWKV, RetNet, xLSTM, TTT")
...
class LIONLayer(nn.Module):
    def __init__(self, dim, nums, window_shape, group_size, direction, shift, operator=None, layer_id=0, n_layer=0):
        ...
        for i in range(len(direction)):
            ...
            block_list.append(get_linear_operator(operator.NAME)(**operator_cfg))
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.blocks):
            ...
            # Ensure dtype match for indexed assignment under mixed precision (e.g., bf16 AMP)
            rhs = x_features.view(-1, x_features.shape[-1])[mappings["win2flat"]]
            if rhs.dtype != x.features.dtype:
                rhs = rhs.to(dtype=x.features.dtype)
            x.features[indices] = rhs
