...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, data_dict):
        ...
        if data_dict.get('gt_boxes', None) is not None:
            select_names = self.class_names + ["Noise"]
            selected = common_utils.keep_arrays_by_name(data_dict['gt_names'], select_names)
            data_dict['gt_boxes'] = data_dict['gt_boxes'][selected]
            data_dict['gt_names'] = data_dict['gt_names'][selected]
            if 'pseudo_idxs' in data_dict:
                data_dict['pseudo_idxs'] = data_dict['pseudo_idxs'][selected]

            gt_classes = np.array([self.class_names.index(n) + 1 if n in self.class_names else 0 for n in data_dict['gt_names']], dtype=np.int32)
    ...
    def collate_batch(self, batch_list, _unused=False):
        ...
        for key, val in data_dict.items():
            try:
                ...
                elif key in ['gt_box_seqs']:
                    max_gt_seq_len = max([len(x) for x in val])
                    batch_gt_boxes_seq = np.ones((batch_size, max_gt_seq_len), dtype=int) * self.pad_token_id
                    for k in range(batch_size):
                        batch_gt_boxes_seq[k, :val[k].__len__()] = val[k]
                    ret[key] = batch_gt_boxes_seq
