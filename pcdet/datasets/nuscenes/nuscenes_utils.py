...
def format_nuscene_results(metrics, class_names, version='default'):
    ...
    # ---------- helpers (simple, no sorting, no NaN logic) ----------
    def _thresholds():
        # take thresholds from the first class that appears in label_aps
        for name in class_names:
            if name in metrics['label_aps']:
                return list(metrics['label_aps'][name].keys())
        return []

    def _collect_row(section_key, class_name, thresholds):
        d = metrics[section_key][class_name]
        return [float(d[th]) for th in thresholds]

    def _render_block(title, short_prefix, section_key, thresholds):
        lines = []
        lines.append("")  # blank line
        lines.append("=" * 70)
        lines.append(f"Per class {title} table")
        header_cols = [f"{short_prefix}@{th}m" for th in thresholds]
        lines.append("%-20s\t%s\t| %-6s" % (
            "Object Class",
            "\t".join(["%-6s" % h for h in header_cols]),
            f"{short_prefix}@All"
        ))
        lines.append("-" * 70)
        ...
        for cname in class_names:
            row = _collect_row(section_key, cname, thresholds)
            table.append(row)
            lines.append(
                "%-20s\t%s\t| %-6.4f" % (
                    cname,
        ...
        # All-classes mean row
        col_means = [float(np.mean([table[i][j] for i in range(len(class_names))]))
                     for j in range(len(thresholds))]
        overall = float(np.mean([v for row in table for v in row]))
        lines.append("-" * 70)
        lines.append(
            "%-20s\t%s\t| %-6.4f" % (
                "All Classes",
                "\t".join([f"{m:6.4f}" for m in col_means]),
                overall
            )
        )
        lines.append("=" * 70)
    ...
    # ---------- append tables (AP, Precision, Recall, F1) ----------
    ths = _thresholds()
    result += _render_block("AP", "AP", "label_aps", ths)
    result += _render_block("Precision", "P", "global_precs", ths)
    result += _render_block("Recall", "R", "global_recs", ths)
    result += _render_block("F1 Score", "F1", "f1_scores", ths)
