...
class DataAugmentor(object):
    ...
    def reorder_object_instances(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.reorder_object_instances, config=config)
        obj_order = config['ORDER']

        if obj_order == 'random':
            obj_indices = np.random.permutation(len(data_dict['gt_boxes']))
        elif obj_order == 'distance':
            box_dists = np.linalg.norm(data_dict['gt_boxes'][:, :3], axis=1)
            obj_indices = np.argsort(box_dists)
        elif obj_order == 'num_points':
            point_masks = roiaware_pool3d_utils.points_in_boxes_cpu(
                data_dict['points'][:, :3], data_dict['gt_boxes'][:, :7]) # bool mask
            num_points_in_each_box = point_masks.sum(axis=1)  # (M,)
            obj_indices = np.argsort(num_points_in_each_box)[::-1]
        else:
            raise NotImplementedError('unknown object order type %s' % obj_order)

        gt_boxes = data_dict['gt_boxes']
        gt_names = data_dict['gt_names']
        data_dict['gt_boxes'] = gt_boxes[obj_indices]
        data_dict['gt_names'] = gt_names[obj_indices]
                
        if 'roi_boxes' in data_dict.keys():
            roi_boxes = data_dict['roi_boxes']
            data_dict['roi_boxes'] = roi_boxes[obj_indices]
        if 'gt_boxes2d' in data_dict:
                data_dict['gt_boxes2d'] = data_dict['gt_boxes2d'][obj_indices]
